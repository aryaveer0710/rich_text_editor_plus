# Rich Text Editor Plugin — Part 3 of 5: Toolbar

## Context

Parts 1-2 created the models, theme, JS bridge, and controller. Now we build the Flutter toolbar widget that sends formatting commands to the editor and reflects the current selection style.

---

## Step 1: `lib/src/toolbar.dart`
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Tooltip(
        ...
        child: Material(
          color: isActive
              ? (activeBackground ?? activeColor.withOpacity(0.1))
              : Colors.transparent,
...
The toolbar is now complete. It listens to the controller, highlights active styles, groups actions with separators, and includes a built-in link dialog.

**Proceed to Part 4 to build the platform-specific editor widgets and the main RichTextEditor.**
